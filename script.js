document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      const babyAge = document.getElementById('babyAge') ? document.getElementById('babyAge').value : '';
      const message = document.getElementById('message').value;
      
      alert('תודה ' + name + '! הודעתך נשלחה בהצלחה. נחזור אליך בהקדם.');
      contactForm.reset();
    });
  }
});
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "שרה מזוז - יועצת שינה משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '4f022c04-dc58-4085-aedb-4d999443e67f',
                    name: data.name || '',
                    email: data.email || '',
